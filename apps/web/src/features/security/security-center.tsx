...
export function SecurityCenter() {
  ...
  const load = useCallback(async () => {
    try {
      const [deviceResult, passkeyResult, totpResult] = await Promise.all([
        ...
        request<{ credentials: Passkey[] }>("/api/v1/auth/passkeys"),
  ...
  async function startTotp() {
    try {
      ...
      setEnrollment(
        await request<Enrollment>("/api/v1/auth/totp/enrollment", {
          method: "POST",
          csrf: true,
        }),
      );
  ...
  return (
    <main id="main-content" className={styles.root}>
      <WorkbenchFrame
        ...
        header={
          <WorkbenchHeader
            ...
            title="账户安全"
            description="先完成清单中的下一项保护，再管理凭据、恢复码和设备会话。"
            actions={
        ...
        context={
          <WorkbenchContextBar
            context={{
              ...
              sync: { label: "实时安全状态", tone: "good" },
              vault: { label: "会话安全" },
            }}
          />
        }
        toolbar={
          <WorkbenchToolbar label="安全工具">
        ...
        master={
          <aside className={styles.masterPane}>
            <div className={styles.paneHeading}>
              <span className={styles.eyebrow}>SECURITY CHECKLIST</span>
              <strong>保护清单</strong>
            </div>
            <nav className={styles.masterNav} aria-label="安全设置分区">
              <button
                className={tab === "checklist" ? styles.activeRow : undefined}
                type="button"
              ...
              >
                ...
                <span>
                  ...
                  <small>
                    {
                      [
                        passkeys.some((item) => item.revoked_at === null),
                        Boolean(totp?.enabled),
                        devices.some(
                          (item) => item.current && item.revoked_at === null,
                        ),
              ...
              <button
                className={tab === "credentials" ? styles.activeRow : undefined}
                type="button"
              ...
              >
                ...
                <span>
                  <strong>登录凭据</strong>
                  <small>
                    {passkeys.filter((item) => item.revoked_at === null).length}{" "}
                    个 Passkey
                  </small>
                </span>
                <ProductTag
              ...
              <button
                className={tab === "devices" ? styles.activeRow : undefined}
                type="button"
                onClick={() => setTab("devices")}
              >
                <AppIcon name="layout-template" size={14} />
                <span>
                  <strong>设备与会话</strong>
                  <small>
                    {devices.filter((item) => item.revoked_at === null).length}{" "}
                    个有效设备
                  </small>
                </span>
              </button>
              <button
                className={tab === "recovery" ? styles.activeRow : undefined}
                type="button"
                onClick={() => setTab("recovery")}
              >
                <AppIcon name="lock" size={14} />
                <span>
                  <strong>认证器与恢复</strong>
                  <small>
                    {totp?.enabled
                      ? `${totp.recovery_codes_remaining} 个恢复码`
                      : "TOTP 未启用"}
                  </small>
                </span>
                <ProductTag tone={totp?.enabled ? "good" : "warn"}>
                  {totp?.enabled ? "已启用" : "待处理"}
                </ProductTag>
              </button>
            </nav>
            <div className={styles.masterHint}>
              恢复码只在生成后显示一次；撤销设备或凭据会立即影响对应会话。
            </div>
          </aside>
        }
        mainLabel="安全设置"
        main={
          <div className={styles.mainPane}>
            ...
            <WorkbenchTabs
              ...
              tabs={[
                { label: "概览", value: "checklist" },
                { label: "凭据", value: "credentials", count: passkeys.length },
                { label: "设备", value: "devices", count: devices.length },
                { label: "恢复", value: "recovery" },
              ]}
              value={tab}
            >
              <WorkbenchTabPanel value="checklist">
                <section
                ...
                >
                  ...
                  <ul className={styles.checklist}>
                    <li
                      data-complete={devices.some(
                        (item) => item.current && item.revoked_at === null,
                      )}
                    >
                      <span>
                        <strong>当前设备会话</strong>
                        <small>确认本次登录设备仍由你控制</small>
                      </span>
                    ...
                    <li
                      data-complete={passkeys.some(
                        (item) => item.revoked_at === null,
                      )}
                    >
                      <span>
                        <strong>抗钓鱼登录凭据</strong>
                        <small>Passkey 可降低密码和钓鱼风险</small>
                      </span>
                      <ProductTag
                        tone={
                          passkeys.some((item) => item.revoked_at === null)
                    ...
                    <li data-complete={Boolean(totp?.enabled)}>
                      <span>
                        <strong>第二验证因素</strong>
                        <small>TOTP 与一次性恢复码用于账户恢复</small>
                      </span>
                      <ProductTag tone={totp?.enabled ? "good" : "warn"}>
                        {totp?.enabled ? "已启用" : "待启用"}
                      </ProductTag>
                    </li>
                  </ul>
                </section>
              </WorkbenchTabPanel>
              <WorkbenchTabPanel value="credentials">
                <section
                  className={styles.dataSection}
                ...
                >
                  <header className={styles.sectionHeader}>
                    <div>
                      <span className={styles.eyebrow}>PASSKEYS</span>
                      <h2>登录凭据</h2>
                    </div>
                    <button
                      className={styles.secondaryButton}
                  ...
                  <p className={styles.muted}>
                    Passkey 由系统认证器保护；名称只用于识别，不会改变凭据本身。
                  </p>
                  {passkeys.length ? (
                      ...
                      {passkeys.map((key) => (
                        <li key={key.id}>
                          <span>
                            ...
                            <small>
                              {key.credential_device_type} ·{" "}
                              {key.revoked_at ? "已撤销" : "当前有效"}
                            </small>
                          </span>
                          <button
                    ...
                    <ProductEmptyState
                      icon="＋"
                      title="还没有 Passkey"
                      description="添加一个容易识别的名称，随后使用系统认证器完成注册。"
                    />
                  )}
                </section>
              </WorkbenchTabPanel>
              <WorkbenchTabPanel value="devices">
                <section
                  className={styles.dataSection}
                  data-testid="security-devices"
                >
                  <header className={styles.sectionHeader}>
                    <div>
                      <span className={styles.eyebrow}>ACTIVE SESSIONS</span>
                      <h2>设备与会话</h2>
                    </div>
                    <ProductTag tone="info">
                      {
                        devices.filter((item) => item.revoked_at === null)
                          .length
                      }{" "}
                      个有效
                    </ProductTag>
              ...
              <WorkbenchTabPanel value="recovery">
                <section
                  className={styles.dataSection}
                  data-testid="security-recovery"
                >
                  <header className={styles.sectionHeader}>
                    <div>
                      <span className={styles.eyebrow}>
                    ...
                    <ProductTag tone={totp?.enabled ? "good" : "warn"}>
                      {totp?.enabled ? "TOTP 已启用" : "TOTP 未启用"}
                    </ProductTag>
                  </header>
                  {totp?.enabled ? (
                    <>
                      <p className={styles.muted}>
                        剩余恢复码：{totp.recovery_codes_remaining}
                        。高风险操作需要当前动态码。
                      </p>
                      <button
                        className={styles.secondaryButton}
          ...
          <div className={styles.inspectorPane}>
            <InspectorSection title="保护摘要">
              <dl className={styles.kvList}>
                ...
                <div>
                  <dt>TOTP</dt>
                  <dd>{totp?.enabled ? "已启用" : "未启用"}</dd>
                </div>
                <div>
                  <dt>恢复码</dt>
                  <dd>{totp?.recovery_codes_remaining ?? 0} 个</dd>
                </div>
              </dl>
            </InspectorSection>
            <InspectorSection title="风险与恢复">
              <p className={styles.muted}>
                撤销设备或 Passkey 会立即影响该凭据对应的会话；TOTP
                关闭会同时废止剩余恢复码。
              </p>
              {recoveryCodes.length > 0 ? (
                <p className={styles.stateGood}>
                  恢复码已生成，请在离开前离线保存。
                </p>
              ) : null}
            </InspectorSection>
      ...
      />
      <WorkbenchSheet
        description="名称只用于识别；随后会调用系统认证器完成 Passkey 注册。"
        onOpenChange={setPasskeySheetOpen}
        open={passkeySheetOpen}
        title="添加 Passkey"
      >
        <form
          className={styles.sheetForm}
          onSubmit={(event) => {
            void registerPasskey(event).then(() => setPasskeySheetOpen(false));
          }}
        >
          ...
          <footer className={styles.sheetActions}>
            <button type="button" onClick={() => setPasskeySheetOpen(false)}>
              取消
            </button>
            <button className={styles.primaryButton}>使用认证器注册</button>
          </footer>
        </form>
      </WorkbenchSheet>
      <WorkbenchSheet
      ...
      >
        {enrollment ? (
          <div className={styles.secretPanel}>
            ...
            <form className={styles.sheetForm} onSubmit={verifyTotp}>
              ...
              />
              <footer className={styles.sheetActions}>
                <button type="button" onClick={() => setTotpSheetOpen(false)}>
                  取消
                </button>
                <button className={styles.primaryButton}>验证并启用</button>
              </footer>
            </form>
          </div>
        ) : totp?.enabled ? (
          <div className={styles.sheetStack}>
            ...
            <form className={styles.sheetForm} onSubmit={regenerateCodes}>
              ...
              <input
                ...
                required
              />
              <button className={styles.secondaryButton}>重新生成恢复码</button>
            </form>
            <form className={styles.sheetForm} onSubmit={disableTotp}>
              ...
              <input
                ...
                required
              />
              <button className={styles.dangerButton}>关闭 TOTP</button>
            </form>
          </div>
        ) : (
          <ProductEmptyState
