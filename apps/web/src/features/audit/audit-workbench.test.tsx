...
describe("Audit workbench", () => {
  it("sends audit pagination filters through the API query option", async () => {
    ...
    await waitFor(() => expect(screen.getAllByText("identity.login").length).toBeGreaterThan(0));
  ...
  it("renders filter master, timeline main, and event inspector without legacy panels", async () => {
    ...
    expect(container.querySelector('[data-testid="audit-filters"]')).toBeTruthy();
    expect(container.querySelector('[data-testid="audit-timeline"]')).toBeTruthy();
    expect(container.querySelector('[data-testid="audit-event-detail"]')).toBeTruthy();
  ...
  it("loads the next cursor from the timeline action", async () => {
    ...
    await waitFor(() => expect(screen.getByText("identity.password_reset")).toBeTruthy());
    expect(request).toHaveBeenCalledWith("/api/v1/audit/me", {
