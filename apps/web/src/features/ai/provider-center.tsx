...
export function ProviderCenter() {
      ...
      try {
        const [providerResult, modelResult, routeResult, budgetResult] =
          await Promise.all([
            ...
            request<Budget>(`/api/v1/workspaces/${selected}/ai/budget`),
          ]);
  ...
  async function createProvider(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await request(`/api/v1/workspaces/${workspaceId}/ai/providers`, {
        method: "POST",
        csrf: true,
        body: JSON.stringify({
  ...
  async function updateBudget(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await request(`/api/v1/workspaces/${workspaceId}/ai/budget`, {
        method: "PUT",
        csrf: true,
        body: JSON.stringify({
  ...
  async function updateModel(event: FormEvent<HTMLFormElement>, model: Model) {
    ...
    const data = new FormData(event.currentTarget);
    try {
      await request(`/api/v1/workspaces/${workspaceId}/ai/models/${model.id}`, {
        method: "PUT",
        csrf: true,
        body: JSON.stringify({
  ...
  async function createRoute(event: FormEvent<HTMLFormElement>) {
    ...
    const data = new FormData(form);
    try {
      await request(`/api/v1/workspaces/${workspaceId}/ai/routes`, {
        method: "POST",
        csrf: true,
        body: JSON.stringify({
          id: crypto.randomUUID(),
  ...
  return (
    ...
    >
      <WorkbenchFrame
        ...
        header={
          <WorkbenchHeader
            ...
            description="Provider、模型能力和任务路由集中治理；密钥只在服务端加密保存。"
            actions={
              <button
          ...
          <WorkbenchContextBar
            context={{
              ...
              permission: selectedWorkspace
                ? {
                    label: canConfigure ? "管理员配置" : "只读状态",
                    tone: canConfigure ? "good" : "warn",
                  }
                : undefined,
              sync: {
                label: online ? "在线" : "离线",
        ...
        toolbar={
          <WorkbenchToolbar label="AI 模型工作区工具">
        ...
        master={
          <aside className={styles.masterPane}>
            ...
            <nav className={styles.masterNav} aria-label="AI 设置分区">
              <button
                className={
                  tab === "providers" ? styles.masterNavActive : undefined
                }
                type="button"
                onClick={() => setTab("providers")}
              >
                <AppIcon name="ai" size={14} />
                Provider <ProductTag>{visibleProviders.length}</ProductTag>
              </button>
              <button
                className={
                  tab === "models" ? styles.masterNavActive : undefined
                }
                type="button"
                onClick={() => setTab("models")}
              >
                <AppIcon name="files" size={14} />
                模型 <ProductTag>{visibleModels.length}</ProductTag>
              </button>
              <button
                className={
                  tab === "routes" ? styles.masterNavActive : undefined
                }
                type="button"
                onClick={() => setTab("routes")}
              >
                <AppIcon name="target" size={14} />
                任务路由 <ProductTag>{routes.length}</ProductTag>
              </button>
              <button
                className={
                  tab === "budget" ? styles.masterNavActive : undefined
                }
                type="button"
            ...
            <div className={styles.masterList}>
              ...
              {visibleProviders.length === 0 ? (
                <ProductEmptyState
                  icon="◇"
                  title="尚未配置 Provider"
                  description="核心学习功能不受影响；需要 AI 时再添加连接。"
                />
              ) : null}
            </div>
            <button
              className={styles.masterCreate}
              type="button"
              onClick={() => setProviderSheetOpen(true)}
            >
              <AppIcon name="plus" size={14} />
          ...
          <div className={styles.mainPane}>
            <WorkbenchActionBar
              secondary={
                <button
                  type="button"
                  onClick={() => void loadProviderData(workspaceId)}
                  disabled={!workspaceId || !online}
                >
                ...
                tab === "providers" ? (
                  selectedProvider ? (
                    ...
                    <button
                      className={styles.primaryButton}
                      type="button"
                      disabled={!online || !canConfigure || !workspaceId}
                      onClick={() => setProviderSheetOpen(true)}
                    >
                      <AppIcon name="plus" size={14} />
                      新增 Provider
            ...
            <WorkbenchTabs
              ...
              tabs={[
                ...
                { label: "模型", value: "models", count: visibleModels.length },
                { label: "任务路由", value: "routes", count: routes.length },
                { label: "预算", value: "budget" },
              ]}
              value={tab}
            >
              <WorkbenchTabPanel value="providers">
                <section className={styles.dataSection}>
                  <header className={styles.sectionHeader}>
                    <div>
                      <span className={styles.eyebrow}>PROVIDER HEALTH</span>
                      <h2>Provider 健康状态</h2>
                    </div>
                    <ProductTag tone={canConfigure ? "good" : "warn"}>
                      {canConfigure ? "可配置" : "只读"}
                    </ProductTag>
                  </header>
                  {selectedProvider ? (
                    <div className={styles.providerDetail}>
                      ...
                      <div className={styles.inlineActions}>
                        ...
                        >
                          {selectedProvider.enabled ? "停用" : "启用"}
                        </button>
                        <button
                          type="button"
                          disabled={!online || !canConfigure}
                    ...
                    <ProductEmptyState
                      icon="◇"
                      title="尚未配置 Provider"
                      description="添加一个公开 HTTPS Provider 后再进行连接检查。"
                    />
                  )}
                </section>
              </WorkbenchTabPanel>
              <WorkbenchTabPanel value="models">
                <section className={styles.dataSection}>
                  <header className={styles.sectionHeader}>
                    <div>
                      <span className={styles.eyebrow}>DISCOVERED MODELS</span>
                      <h2>已发现模型</h2>
                    </div>
                    <ProductTag>{visibleModels.length} 个</ProductTag>
                  </header>
                  {visibleModels.length ? (
                    ...
                    >
                      ...
                      {visibleModels.map((model) => {
                        const provider = visibleProviderById.get(
                          model.provider_id,
                        );
                        return (
                          <div
                          ...
                          >
                            <span>
                              ...
                              <small>
                                {provider?.name ?? "未知 Provider"} ·{" "}
                                {model.source}
                              </small>
                            </span>
                            <span>
                              {model.enabled ? "启用" : "停用"} · JSON{" "}
                              {model.supports_json ? "是" : "否"} · Stream{" "}
                              {model.supports_stream ? "是" : "否"}
                            </span>
                            <span>{model.context_window ?? "未声明"}</span>
                            <button
                              type="button"
                              onClick={() => setModelSheet(model)}
                            >
                              编辑能力
                            </button>
                          </div>
                    ...
                    <ProductEmptyState
                      icon="⌁"
                      title="尚未发现模型"
                      description="选择一个已启用 Provider 执行测试并发现模型。"
                    />
                  )}
                </section>
              </WorkbenchTabPanel>
              <WorkbenchTabPanel value="routes">
                <section className={styles.dataSection} id="ai-route-center">
                  <header className={styles.sectionHeader}>
                    <div>
                  ...
                  {routes.length ? (
                      ...
                      {routes.map((route) => (
                        <li key={route.id}>
                          <span>
                            ...
                            <small>
                              {route.task_type} · {route.model_ids.length}{" "}
                              个主备模型 · 输入 {route.max_input_tokens} / 输出{" "}
                    ...
                    <ProductEmptyState
                      icon="⇢"
                      title="尚未配置任务路由"
                      description="发现并启用模型后，为常用任务建立明确路由。"
                    />
                  )}
                </section>
              </WorkbenchTabPanel>
              <WorkbenchTabPanel value="budget">
                <section className={styles.dataSection} id="ai-budget-center">
                  <header className={styles.sectionHeader}>
                    <div>
                      <span className={styles.eyebrow}>RESOURCE GUARDRAIL</span>
                      <h2>AI Token 使用上限</h2>
                    </div>
                    <ProductTag tone="info">
                      {budget?.monthly_token_budget ?? "未设置"}
                    </ProductTag>
                  </header>
                  <p className={styles.muted}>
                    留空表示不设置月度 Token 上限；正式运行会在服务端再次校验。
                  </p>
                  <button
                    className={styles.secondaryButton}
                    type="button"
            ...
            <InspectorSection title="当前 Workspace">
              <dl className={styles.kvList}>
                ...
                <div>
                  <dt>Provider</dt>
                  <dd>
                    {visibleProviders.length} 个 ·{" "}
                    {visibleProviders.filter((item) => item.enabled).length}{" "}
                    个启用
                  </dd>
                </div>
                <div>
                  <dt>月度 Token</dt>
                  <dd>{budget?.monthly_token_budget ?? "未设置"}</dd>
                </div>
              </dl>
            </InspectorSection>
            <InspectorSection title="安全边界">
              <p className={styles.inspectorCopy}>
                密钥只在服务端信封加密保存，不进入浏览器响应、IndexedDB、导出或日志。连接测试必须由你明确触发。
              </p>
              {!online ? (
                <p className={styles.stateWarn} role="status">
                  当前离线：Provider 配置只读，核心学习功能仍可用。
                </p>
              ) : selectedWorkspace && !canConfigure ? (
                <p className={styles.stateWarn} role="status">
                  当前角色无权配置 Provider；可继续查看核心学习内容。
                </p>
              ) : null}
            </InspectorSection>
      ...
      />
      <WorkbenchSheet
        description="加密保存服务地址、密钥和连接策略；密钥不会回显。"
        onOpenChange={setProviderSheetOpen}
        open={providerSheetOpen}
        title="新增 OpenAI-compatible Provider"
      >
        <form
          className={styles.sheetForm}
          onSubmit={(event) => {
            void createProvider(event).then(() => setProviderSheetOpen(false));
          }}
        >
          ...
          <div className={styles.formGrid}>
            ...
            <label htmlFor="provider-retries">
              最大重试次数
              <input
                id="provider-retries"
                name="max_retries"
      ...
      <WorkbenchSheet
        description="留空表示不设置月度 Token 上限。"
      ...
      >
        <form
          ...
          className={styles.sheetForm}
          onSubmit={(event) => {
            void updateBudget(event).then(() => setBudgetSheetOpen(false));
          }}
        >
          <label htmlFor="monthly-token-budget">
            Token 上限
            <input
          ...
          <footer className={styles.sheetActions}>
            <button type="button" onClick={() => setBudgetSheetOpen(false)}>
              取消
            </button>
            <button
              className={styles.primaryButton}
      ...
      >
        <form
          className={styles.sheetForm}
          onSubmit={(event) => {
            void createRoute(event).then(() => setRouteSheetOpen(false));
          }}
        >
          <label>
            路由名称
          ...
          <fieldset>
            <legend>模型顺序</legend>
            {visibleModels.map((model) => (
              <label className={styles.checkRow} key={model.id}>
          ...
          <footer className={styles.sheetActions}>
            <button type="button" onClick={() => setRouteSheetOpen(false)}>
              取消
            </button>
            <button
              className={styles.primaryButton}
      ...
      <WorkbenchSheet
        description="模型能力来自明确触发的 Provider 发现结果。"
        onOpenChange={(open) => {
          if (!open) setModelSheet(null);
        }}
        open={modelSheet !== null}
        title="编辑模型能力"
      >
        {modelSheet ? (
          <form
            className={styles.sheetForm}
            onSubmit={(event) => {
              void updateModel(event, modelSheet).then(() =>
                setModelSheet(null),
              );
            }}
          >
            <label>
              ...
              <input
                name="display_name"
                defaultValue={modelSheet.display_name}
                maxLength={255}
                required
              />
            </label>
            <label className={styles.checkRow}>
              <input
                name="enabled"
                type="checkbox"
                defaultChecked={modelSheet.enabled}
              />
              启用
            </label>
            <label className={styles.checkRow}>
              <input
                name="supports_json"
                type="checkbox"
                defaultChecked={modelSheet.supports_json}
              />
              支持结构化 JSON
            </label>
            <label className={styles.checkRow}>
              <input
                ...
                type="checkbox"
                defaultChecked={modelSheet.supports_stream}
              />
              支持流式输出
            </label>
            <label>
              ...
              <input
                ...
                min={1}
                max={10_000_000}
                defaultValue={modelSheet.context_window ?? ""}
              />
            </label>
            <footer className={styles.sheetActions}>
              <button type="button" onClick={() => setModelSheet(null)}>
                取消
              </button>
              <button
                className={styles.primaryButton}
