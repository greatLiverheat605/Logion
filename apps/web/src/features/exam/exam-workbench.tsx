  ...
  return (
    <div className={styles.master} data-testid="exam-list">
      <header className={styles.paneHeader}>
        <div>
          <p className={styles.eyebrow}>EXAM MASTER</p>
          <h2>考试列表</h2>
        </div>
        <span
          className={styles.count}
          aria-label={`${data.visibleExams.length} 项考试`}
        >
          {data.visibleExams.length}
        </span>
      </header>
      <button
      ...
      <div aria-label="当前 Space 的考试" className={styles.examList}>
        {data.visibleExams.length ? (
            ...
            return (
              ...
              >
                ...
                <span className={styles.examCopy}>
                  ...
                  <small>
                    {examCountdown(exam.payload.exam_at)} ·{" "}
                    {formatDate(exam.payload.exam_at)}
                  </small>
                  <span className={styles.rowTags}>
                    <ProductTag
                      tone={
                        exam.entity.sync_status === "clean" ? "good" : "info"
                      }
                    >
                      {exam.entity.sync_status === "clean"
                        ? "已同步"
                        : "待同步"}
                    </ProductTag>
                    {exam.payload.target_score !== null ? (
                      <ProductTag tone="warn">
                        目标 {exam.payload.target_score}
                      </ProductTag>
                    ) : null}
                  </span>
                </span>
          ...
          <EmptyPane
            description="创建考试后，这里会形成可选择的备考目标。"
            title="当前 Space 还没有考试"
          />
        )}
      </div>
...
function CoverageMain({
  ...
  const subjectRows = data.visibleSubjects.filter(
    (subject) => subject.payload.exam_id === selectedExam?.entity.entity_id,
  );
  const nodeRows = data.visibleNodes.filter((node) =>
    subjectRows.some(
      (subject) => subject.entity.entity_id === node.payload.subject_id,
    ),
  );
  const mocks = data.visibleMocks.filter(
    (mock) => mock.payload.exam_id === selectedExam?.entity.entity_id,
  );
  const weakNodes = [...nodeRows]
    .filter((node) => node.payload.coverage_status !== "covered")
    .sort((a, b) => b.payload.importance - a.payload.importance);
  const coverageText =
  ...
  return (
    <div className={styles.main}>
      <StatusLine>
        {"考试数据来自当前 Space 的加密本地记录；每次写入后会尝试同步。"}
      </StatusLine>
      <section className={styles.examHero} aria-label="当前考试摘要">
        <div>
          ...
          <p>
            {selectedExam
              ? `${formatDate(selectedExam.payload.exam_at)} · ${examCountdown(selectedExam.payload.exam_at)}`
              : "从考试日期、科目权重和大纲覆盖开始。"}
          </p>
        </div>
        <div className={styles.heroMetric}>
          <strong>{selectedExam ? coverageText : "—"}</strong>
          <span>大纲覆盖</span>
        </div>
      </section>
      ...
      >
        <header className={styles.sectionHeader}>
          ...
          <div className={styles.sectionActions}>
            <span className={styles.sectionMeta}>
              {data.coveredNodes}/{nodeRows.length || 0} 个节点
            </span>
            <button
              className={styles.sectionAction}
              disabled={!selectedExam}
              onClick={() => onOpenSheet("subject")}
              type="button"
            >
              添加科目
            </button>
        ...
        <div className={styles.metricStrip}>
          ...
          <div>
            <span>最近得分率</span>
            <strong>
              {data.normalizedScores.length
                ? `${Math.round(data.latestNormalizedScore)}%`
                : "—"}
            </strong>
          </div>
        </div>
        <div className={styles.subjectList}>
          {subjectRows.length ? (
            subjectRows.map((subject) => {
              const nodes = nodeRows.filter(
                (node) => node.payload.subject_id === subject.entity.entity_id,
              );
              const covered = nodes.filter(
                (node) => node.payload.coverage_status === "covered",
              ).length;
              ...
              return (
                ...
                >
                  <div>
                    <strong>{subject.payload.name}</strong>
                    <small>
                      权重{" "}
                      {(subject.payload.weight_basis_points / 100).toFixed(0)}%
                      · {covered}/{nodes.length} 节点已覆盖
                    </small>
                  </div>
                  <div
                    aria-label={`${subject.payload.name} 覆盖率`}
                    aria-valuemax={100}
                    aria-valuemin={0}
            ...
            <EmptyPane
              description="为当前考试添加科目后，覆盖率会按真实大纲节点计算。"
              title="还没有科目权重"
            />
          )}
        </div>
      ...
      >
        <header className={styles.sectionHeader}>
          <div>
            ...
            <h2>考试大纲</h2>
            <p>覆盖状态由本人显式记录，不把推测当作掌握。</p>
          </div>
          <button
            className={styles.sectionAction}
            disabled={!selectedExam || !data.visibleSubjects.length}
            onClick={() => {
              actions.setSyllabusSubjectId(
                subjectRows[0]?.entity.entity_id ?? "",
              );
              onOpenSheet("syllabus");
            }}
            type="button"
          >
            添加大纲节点
          ...
          {nodeRows.length ? (
            nodeRows.map((node) => {
              const subject = subjectRows.find(
                (item) => item.entity.entity_id === node.payload.subject_id,
              );
              const labels = {
                covered: "已覆盖",
                in_progress: "进行中",
              ...
              return (
                <div className={styles.syllabusRow} key={node.entity.entity_id}>
                  <span className={styles.treeIndent} aria-hidden="true">
                    {node.payload.parent_id ? "↳" : "•"}
                  </span>
                  <div>
                    <strong>{node.payload.title}</strong>
                    <small>
                      {subject?.payload.name ?? "科目"} · 重要度{" "}
                      {node.payload.importance}/5
                    </small>
                  </div>
                  <ProductTag
                    tone={
                  ...
                  >
                    {labels[node.payload.coverage_status]}
                  </ProductTag>
                </div>
              );
            })
            ...
            <EmptyPane
              description="添加科目后建立树状大纲；父节点关系会在本地提交前校验。"
              title="还没有大纲节点"
            />
          )}
        </div>
      ...
      <section
        className={styles.section}
        data-testid="exam-mocks"
        aria-label="模考与成绩"
      >
        <header className={styles.sectionHeader}>
          <div>
            <p className={styles.eyebrow}>MOCKS</p>
            <h2>模考与成绩</h2>
            <p>安排限时练习，完成后追加真实成绩记录。</p>
          </div>
          <div className={styles.sectionActions}>
            <button
              className={styles.sectionAction}
              disabled={!selectedExam}
              onClick={() => onOpenSheet("mock")}
              type="button"
            >
              安排模考
            </button>
        ...
        <div className={styles.mockList}>
          {mocks.length ? (
            mocks.map((mock) => {
              const score = scoreForMock(
                data.visibleScores,
                mock.entity.entity_id,
              );
              return (
                <div className={styles.mockRow} key={mock.entity.entity_id}>
                  <div>
                    <strong>{mock.payload.title}</strong>
                    <small>
                      {Math.round(mock.payload.duration_limit_seconds / 60)}{" "}
                      分钟 ·{" "}
                      {score
                        ? `最近 ${score.payload.score}/${score.payload.score_scale_max}`
                        : "尚未记录成绩"}
                    </small>
                  </div>
                  <button
                    aria-label={
                      score
                        ? `追加 ${mock.payload.title} 成绩`
                        : `录入 ${mock.payload.title} 成绩`
                    }
            ...
            <EmptyPane
              description="安排第一场模考后，成绩趋势和用时会在这里持续累积。"
              title="尚未安排模考"
            />
          )}
        </div>
      ...
      <section
        className={styles.section}
        data-testid="exam-weaknesses"
        aria-label="薄弱项"
      >
        <header className={styles.sectionHeader}>
          <div>
            <p className={styles.eyebrow}>WEAKNESS</p>
            <h2>薄弱项</h2>
            <p>根据未覆盖节点和重要度排序，不生成虚构的错题结论。</p>
          </div>
        </header>
        {weakNodes.length ? (
          ...
          <EmptyPane
            description="当大纲节点尚未覆盖时，这里会按重要度提示需要补强的内容。"
            title="暂未形成薄弱项"
          />
        )}
      </section>
...
function ExamInspector({
  ...
  const subjects = data.visibleSubjects.filter(
    (item) => item.payload.exam_id === selectedExam?.entity.entity_id,
  );
  const nodes = data.visibleNodes.filter((item) =>
    subjects.some(
      (subject) => subject.entity.entity_id === item.payload.subject_id,
    ),
  );
  return (
    <div className={styles.inspector} data-testid="exam-inspector">
      <header className={styles.inspectorHeader}>
        <p className={styles.eyebrow}>EXAM INSPECTOR</p>
        <h2>{selectedExam?.payload.title ?? "考试详情"}</h2>
        <p>
          {selectedExam
            ? "当前考试的范围、目标与本地同步边界。"
            : "选择一场考试查看详情。"}
        ...
        <>
          <InspectorSection title="考试范围">
            <dl className={styles.metaList}>
              <div>
                <dt>日期</dt>
                <dd>{formatDate(selectedExam.payload.exam_at)}</dd>
              </div>
              <div>
                <dt>倒计时</dt>
                <dd>{examCountdown(selectedExam.payload.exam_at)}</dd>
              </div>
              <div>
                <dt>目标分</dt>
                <dd>
                  {selectedExam.payload.target_score === null
                    ? "未设置"
                    : `${selectedExam.payload.target_score} / ${selectedExam.payload.score_scale_max}`}
                </dd>
              </div>
              <div>
                <dt>科目</dt>
                <dd>{subjects.length} 个</dd>
              </div>
              <div>
                <dt>大纲</dt>
          ...
          <InspectorSection title="权限与同步">
            <dl className={styles.metaList}>
              ...
              <div>
                <dt>记录状态</dt>
                <dd>
                  {selectedExam.entity.sync_status === "clean"
                    ? "已同步"
                    : "等待同步"}
                </dd>
              </div>
          ...
          <p className={styles.inspectorNote}>
            AI
            只能解释大纲或提供规划草稿；考试、科目、模考和成绩由本人明确创建。
          </p>
        </>
      ) : (
        <EmptyPane
          description="选择 Master 中的考试，查看目标分、权限与同步状态。"
          title="没有选中的考试"
        />
      )}
    </div>
...
function ContextToolbar({
  ...
  return (
    <WorkbenchToolbar label="备考上下文操作">
      <WorkbenchSelect
      ...
      <WorkbenchSelect
        ...
        options={context.spaces.map((item) => ({
          label: item.name,
          value: item.id,
        }))}
        placeholder="Space"
...
function UnlockSheet({
  ...
  return (
    <WorkbenchSheet
      description="口令只在当前应用会话内存中使用；考试数据保持端侧加密。"
      footer={
        <>
          <button
...
function FormSheet({
  ...
  actions: ExamWorkbenchActions;
  context: ExamWorkbenchContext;
  kind: "exam" | "subject" | "syllabus" | "mock" | "score";
  onOpenChange: (open: boolean) => void;
  open: boolean;
  data: ExamWorkbenchData;
  ...
  const submit = {
    exam: actions.createExam,
    subject: actions.createSubject,
    syllabus: actions.createSyllabusNode,
    mock: actions.createMockExam,
    score: actions.createScoreRecord,
  }[kind];
  return (
    <WorkbenchSheet
      description="低频字段在此局部编辑；保存后会加密写入本地并尝试同步。"
      footer={
        <>
          <button
            className={styles.secondaryButton}
    ...
    >
      ...
      >
        {kind === "exam" ? (
          <>
            ...
            <fieldset className={styles.choiceField}>
              ...
              <label>
                <input
                  checked={dateStatus === "scheduled"}
                  name="date_status"
                  onChange={() => {
                    setDateStatus("scheduled");
                    actions.setDateStatus("scheduled");
                  }}
                  type="radio"
                  value="scheduled"
                />{" "}
              ...
              <label>
                <input
                  checked={dateStatus === "undetermined"}
                  name="date_status"
                  onChange={() => {
                    setDateStatus("undetermined");
                    actions.setDateStatus("undetermined");
                  }}
                  type="radio"
                  value="undetermined"
                />{" "}
            ...
            {dateStatus === "scheduled" ? (
              <>
                <label htmlFor={`${formId}-at`}>考试时间（本地时区）</label>
                <input
                  id={`${formId}-at`}
                  name="exam_at"
                  required
            ...
            <div className={styles.formGrid}>
              <label htmlFor={`${formId}-target`}>目标分（可选）</label>
              <input
                id={`${formId}-target`}
                min={0}
                name="target_score"
        ...
        {kind === "subject" ? (
          <>
            <label htmlFor={`${formId}-exam`}>所属考试</label>
            <select
              defaultValue={
                selectedExamId ?? data.primaryExam?.entity.entity_id ?? ""
              }
              id={`${formId}-exam`}
              name="exam_id"
              required
        ...
        {kind === "syllabus" ? (
          <>
            <label htmlFor={`${formId}-subject`}>所属科目</label>
            <select
              id={`${formId}-subject`}
              name="subject_id"
              required
              ...
              onChange={(event) =>
                actions.setSyllabusSubjectId(event.target.value)
              }
            >
              <option value="">请选择</option>
              {data.visibleSubjects.map((subject) => (
                <option
                  key={subject.entity.entity_id}
            ...
            <select id={`${formId}-parent`} name="parent_id">
              ...
              {data.visibleNodes
                .filter(
                  (node) =>
                    node.payload.subject_id === context.syllabusSubjectId,
                )
                .map((node) => (
                  <option
                    key={node.entity.entity_id}
        ...
        {kind === "mock" ? (
          <>
            <label htmlFor={`${formId}-mock-exam`}>所属考试</label>
            <select
              defaultValue={
                selectedExamId ?? data.primaryExam?.entity.entity_id ?? ""
              }
...
export function ExamWorkbench({ actions, context, data }: ExamWorkbenchProps) {
  const [selectedId, setSelectedId] = useState<string | null>(
    data.primaryExam?.entity.entity_id ?? null,
  );
  const [sheet, setSheet] = useState<
    "exam" | "subject" | "syllabus" | "mock" | "score" | "unlock" | null
  >(null);
  const unlockButtonRef = useRef<HTMLButtonElement>(null);
  const effectiveSelectedId =
    selectedId &&
    data.visibleExams.some((exam) => exam.entity.entity_id === selectedId)
      ? selectedId
      : (data.primaryExam?.entity.entity_id ??
        data.visibleExams[0]?.entity.entity_id ??
        null);
  const selectedExam =
    data.visibleExams.find(
      (exam) => exam.entity.entity_id === effectiveSelectedId,
    ) ?? null;
  const primary = context.unlocked ? (
    <button
      ...
      onClick={() => setSheet("exam")}
      type="button"
    >
      <AppIcon name="plus" size={16} /> 创建考试
    </button>
  ) : (
    <button
      className={styles.primaryButton}
    ...
    <main className={styles.root} id="main-content">
      <WorkbenchFrame
        ...
        header={
          <WorkbenchHeader
            actions={primary}
            description="Exam · 以最近考试为中心安排科目覆盖、大纲推进和模考反馈；所有对象在当前 Space 内完成。"
            eyebrow="EXAM · PREPARATION WORKBENCH"
            title="围绕最近考试推进覆盖与模考"
          />
        }
        initialPane="master"
      ...
      {sheet && sheet !== "unlock" ? (
        <FormSheet
          ...
          open
          selectedExamId={selectedExam?.entity.entity_id}
        />
      ) : null}
    </main>
  );
