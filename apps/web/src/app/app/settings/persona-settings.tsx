...
const SECTION_META: Readonly<
  Record<
    SettingsSection,
    {
      icon: "target" | "sun" | "timer" | "layout-template";
      label: string;
      description: string;
    }
  >
> = {
  persona: {
    description: "决定优先显示哪些工作区入口。",
    icon: "target",
    label: "学习画像",
  },
  appearance: {
    description: "控制主题与显示表面。",
    icon: "sun",
    label: "界面与交互",
  },
  interaction: {
    description: "查看操作方式和系统反馈偏好。",
    icon: "timer",
    label: "操作偏好",
  },
  navigation: {
    description: "确认画像保留的系统入口。",
      ...
      <header className={styles.sectionHeader}>
        <div>
          ...
          <p className={styles.sectionDescription}>
            画像只调整导航优先级，不改变 Workspace、Space 或任何权限。
          </p>
        </div>
      </header>
        ...
        {personas.map((persona) => {
          ...
          return (
            <li key={persona.id}>
              ...
              >
                <span aria-hidden="true" className={styles.personaGlyph}>
                  {personaIcon(persona)}
                </span>
                <span className={styles.rowCopy}>
                  <strong>
                    ...
                    <span aria-hidden="true">
                      {persona.isBuiltin ? " · 官方画像" : " · 自定义"}
                    </span>
                  </strong>
                  <small>
                    {persona.description} · {persona.routes.length} 个入口
                  </small>
                </span>
                {selected ? (
                  <span className={styles.selectedMark}>当前使用</span>
                ) : null}
              </button>
              {!persona.isBuiltin ? (
                <button
  ...
  const main =
    section === "persona" ? (
      ...
      <section className={styles.section} data-testid="settings-appearance">
        <header className={styles.sectionHeader}>
          <div>
            <span className={styles.kicker}>SURFACE & THEME</span>
            <h2>界面与交互</h2>
            <p className={styles.sectionDescription}>
              主题切换立即生效，并保留在本机下次访问时使用。
            </p>
          </div>
        </header>
        <div className={styles.themeControl}>
          ...
          <ThemeToggle className={styles.secondaryAction} />
        </div>
        <div className={styles.settingRow}>
          <AppIcon className={styles.settingIcon} name="shield" size={16} />
          <span className={styles.rowCopy}>
            <strong>中性表面</strong>
            <small>页面使用稳定的中性层级，品牌色只强调当前动作和状态。</small>
          </span>
        </div>
      </section>
    ) : section === "interaction" ? (
      <section className={styles.section} data-testid="settings-interaction">
        <header className={styles.sectionHeader}>
          <div>
            <span className={styles.kicker}>INTERACTION</span>
            <h2>操作偏好</h2>
            <p className={styles.sectionDescription}>
              高频操作优先留在当前工作区，低频设置通过 Sheet 打开。
            </p>
          </div>
        </header>
        <ul className={styles.settingList}>
          <li className={styles.settingRow}>
            <AppIcon className={styles.settingIcon} name="search" size={16} />
            <span className={styles.rowCopy}>
              <strong>命令栏</strong>
              <small>使用顶部搜索打开页面、命令和常用恢复动作。</small>
            </span>
            <kbd>Ctrl K</kbd>
          </li>
          <li className={styles.settingRow}>
            <AppIcon className={styles.settingIcon} name="target" size={16} />
            <span className={styles.rowCopy}>
              <strong>聚焦上下文</strong>
              <small>
                工作区、Space、Persona、权限和 Vault 在当前页面持续回显。
              </small>
            </span>
          </li>
          <li className={styles.settingRow}>
            <AppIcon className={styles.settingIcon} name="timer" size={16} />
            <span className={styles.rowCopy}>
              <strong>减少动态效果</strong>
              <small>系统的 reduced-motion 偏好会自动禁用非必要动画。</small>
            </span>
          </li>
        </ul>
      </section>
      ...
      <section className={styles.section} data-testid="settings-navigation">
        <header className={styles.sectionHeader}>
          <div>
            <span className={styles.kicker}>NAVIGATION CONTRACT</span>
            <h2>导航入口</h2>
            <p className={styles.sectionDescription}>
              每日、设置、个人和帮助始终保留，其余入口由当前画像决定。
            </p>
          </div>
        </header>
        <ul className={styles.routeList} aria-label="当前画像导航入口">
          {ALL_ROUTES.map((route) => (
            <li className={styles.routeRow} key={route}>
              <AppIcon
                name={REQUIRED_ROUTES.has(route) ? "shield" : "layout-template"}
                size={15}
              />
              <span className={styles.rowCopy}>
                <strong>
                  {ROUTE_LABELS[route]}
                  {REQUIRED_ROUTES.has(route) ? " · 必需" : ""}
                </strong>
  ...
  return (
    <>
      <main
        ...
        data-testid="settings-workbench"
        id="main-content"
      >
        <WorkbenchFrame
          context={
            <WorkbenchContextBar
              context={{
                persona: activePersona
                  ...
                  : undefined,
                permission: { label: "仅个人偏好", tone: "good" },
                sync: {
                  label: isLoading ? "正在同步" : "已同步",
                  tone: isLoading ? "warn" : "good",
                },
                vault: { label: "本机偏好" },
              }}
            />
          ...
          header={
            <WorkbenchHeader
              actions={
                <span className="product-tag tone-info">账户级偏好</span>
              }
              description="把常用学习场景、主题和导航入口整理成一套可持续使用的工作台。"
          ...
          inspector={
            <aside
              ...
              data-testid="settings-inspector"
            >
              <InspectorSection title="当前偏好">
                <dl className={styles.inspectorList}>
                  <div>
                    <dt>当前画像</dt>
                    <dd>{activePersona?.name ?? "正在读取"}</dd>
                  </div>
                  <div>
                    <dt>可见入口</dt>
                    <dd>{activePersona?.routes.length ?? 0} 个路由</dd>
                  </div>
                  <div>
                    <dt>保存范围</dt>
                    <dd>当前账户</dd>
              ...
              <InspectorSection title="安全边界">
                <p>
                  偏好只控制导航和显示，不改变 Workspace 成员角色、Space 权限或
                  Vault 加密边界。
                </p>
          ...
          label="个人设置工作台"
          main={
            <div data-testid="settings-main" className={styles.main}>
              <div className={styles.mainHeader}>
                ...
                <span className="product-tag tone-good">
                  {isLoading ? "读取中" : "已同步"}
                </span>
              </div>
              <WorkbenchActionBar
          ...
          mainLabel="偏好设置"
          master={
            <aside className={styles.master} data-testid="settings-master">
              <div className={styles.masterHeader}>
                <span className={styles.eyebrow}>SETTINGS INDEX</span>
                <h2>设置分组</h2>
              </div>
              <nav aria-label="设置分组" className={styles.masterNav}>
                {(
                  Object.entries(SECTION_META) as Array<
                    [SettingsSection, (typeof SECTION_META)[SettingsSection]]
                  >
                ).map(([id, value]) => (
                  <button
                  ...
                  >
                    <AppIcon name={value.icon} size={16} />
                    <span className={styles.rowCopy}>
              ...
              <p className={styles.masterHint}>
                设置在当前账户范围内生效；Workspace 权限请前往成员治理页面调整。
              </p>
            </aside>
          }
          masterLabel="设置分组"
          toolbar={
            <WorkbenchToolbar label="设置状态">
              <span className={styles.status} aria-live="polite">
                {pendingId ? "正在保存偏好…" : status}
              </span>
            </WorkbenchToolbar>
          }
        />
      ...
      <WorkbenchSheet
        description="自定义画像会保存到当前账户，并始终保留每日、设置、个人和帮助入口。"
      ...
      >
        <form id="custom-persona-form" onSubmit={(event) => void create(event)}>
          ...
          <select id="custom-persona-icon" name="icon" defaultValue={ICONS[0]}>
            {ICONS.map((icon) => (
