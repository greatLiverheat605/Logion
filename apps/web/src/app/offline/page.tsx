    ...
    <PublicFlowShell wide>
      ...
      <div className={styles.stack}>
        <section data-testid="offline-state">
          <PublicFlowState
            icon="!"
            title={online ? "网络已恢复，等待重新连接" : "网络连接不可用"}
            tone={online ? "success" : "warning"}
          >
            <p aria-live="polite">
              {online
                ? "可以重试打开目标页面；服务器状态仍需重新读取。"
                : "当前请求无法访问服务器，已打开的本地页面仍可按其能力继续工作。"}
            </p>
          </PublicFlowState>
        </section>
        ...
        <section className={styles.region} data-testid="offline-local">
          ...
          <p className={styles.muted}>
            已打开并解锁 Vault 的页面可以把受保护修改写入本机 IndexedDB 与
            Outbox；认证恢复、成员权限更新和云端 AI 需要重新联网。
          </p>
          <p className={`${styles.notice} ${styles.noticeSuccess}`}>
            本地写入不代表云端成功。网络恢复后请在同步工作台确认
            Outbox、附件和冲突。
          </p>
        </section>
