...
test("Exam completes the protected planning and mock-exam workflow", async ({
  accountState,
  page,
}) => {
  ...
  await page
    .getByTestId("exam-coverage")
    .getByRole("button", { name: "添加科目" })
    .click();
  ...
  await page
    .getByTestId("exam-syllabus")
    .getByRole("button", { name: "添加大纲节点" })
    .click();
  ...
  await page
    .getByTestId("exam-mocks")
    .getByRole("button", { name: "安排模考" })
    .click();
  ...
  await page
    .getByTestId("exam-mocks")
    .getByRole("button", { name: "记录成绩" })
    .click();
  const scoreSheet = page.getByRole("dialog", { name: "记录成绩" });
  await scoreSheet
    .getByLabel("已完成模考")
    .selectOption({ label: "第一次全真模考" });
