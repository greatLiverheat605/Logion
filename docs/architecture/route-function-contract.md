...
本文维护现有页面的职责、状态与恢复边界。面向用户的操作步骤见 [操作手册](../user-guide.md)。
...
- Records 选段使用已有 `topic` / `quiz_item` 加密 mutation；必须经可编辑表单确认。知识点说明或题目解析保存原笔记标题与选段快照，同一空间内创建；题目依赖未同步知识点的 Outbox operation。长度遵循现有字段限制，不扩展 sync-v1 或来源关联 schema。
- Sync push 与非空 pull 在现有服务端事务中写入 `sync` 类通知，仅保存数量/状态。Push 按 epoch、operation ID 与规范化结果去重，纯 duplicate 不新增通知；Pull 按 epoch、device、cursor 范围去重。升级控制和回滚路径不保留回执。成功解决冲突计入推送摘要。拉取回执不等同于客户端已应用数据。
- Worker 的 `review_reminders` 队列每分钟以键集分页检查至多 100 个工作区/用户组合，轮转后继续扫描。仅统计当前授权、未删除且 `scheduled` / `due` 的到期计划；按用户时区的日历日及现有通知唯一键并发去重。学习类别关闭时不写入，跨午夜安静时间会延期，首尾相同视为无静默。每工作区/用户每天至多一条站内汇总，不更改复习排期，不增加外呼。
- 通知跳转只允许固定复习/同步路由；`workspace`、`space` 参数必须匹配服务端返回的可访问列表。日历排除已删除知识点；新迭代没有新增 API 字段、迁移、生产开关或数据保留策略。
