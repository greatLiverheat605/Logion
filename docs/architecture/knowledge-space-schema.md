...
本文记录 SourceExcerpt 与 KnowledgeCitation 的设计约束。当前实现以 Alembic 迁移、ORM 和数据库约束测试为准；部署前应独立验证迁移、备份与恢复。
...
PostgreSQL CHECK 会将 NULL 视为未违反约束。页码和字符范围必须显式要求 start/end 同时非空，再验证范围；只有一端的定位必须拒绝。迁移检查应覆盖该三值逻辑边界，并核对约束名和 SQLSTATE。
