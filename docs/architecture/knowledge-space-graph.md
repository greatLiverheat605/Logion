...
服务端负责权限、查询范围与响应限制，浏览器负责布局和交互。本文不将规划中的语义检索作为已实现功能。
...
浏览器负责图布局、节点聚焦、边语义和移动端列表/树的投影；服务端不返回布局坐标，也不把 Canvas 作为唯一操作方式。前端变更应通过 1440/1024/390px、键盘/axe、loading/empty/locked/online-only 和存储型 XSS 门禁。
