...
> 原文件字节 SHA-256：`aee19c11c9681f5cdbc9f8fd74e6da6081797a9e91618c2e6a15b3a38f2c58f3`。正文仅调整移入本目录后的相对链接与 Markdown 格式，历史失败、授权与未运行项保留。
...
## FABLE M6 D-18 技术验收与最终候选（2026-09-09）

- 固定候选 `1e1c6fd46d72af6054702a270fd07e671c0b7525` 已提交推送；草稿 PR #233
  共 15 提交、175 文件。D-18 获批策略完整实现并验收，普通旧端同步可用，删除要求
  升级时保留内容，新端从原 Outbox/游标/Vault 恢复。本次没有新增 migration。
- 完整 `pnpm ci:fast` 通过：Web 433、offline 74、Python 614 passed / 133 deselected，
  类型、lint、格式、构建、合同生成无差异；独占集成 133 passed / 578 deselected，
  迁移/清理通过。17 项真实旧源码兼容、46 份新版 API 与 5 份实际旧 API 响应校验通过。
  旧 API 使用当前锁定依赖、ASGI 与独立数据库，非旧生产二进制或生产网络验收。
- 同 SHA PR run `34360935647` 的 fast/integration/browser 及 mobile run
  `34360935596` 全部 success；浏览器 135 passed、10 既定 skipped、零失败或 flaky。
  制品身份见[发布准备](.././V021_M6_RELEASE_PREPARATION.md)。初次覆盖率路径扫描失败、
  约正负 11 秒时钟调整及认证/邮件历史失败保留，未放宽门禁或修改相关生产逻辑。
- 原主树基线、空暂存区、T-04 保护与用户数据保持；检查后状态仅记录于本地，已绿候选
  clean 不变。M5 按批准范围完成，真实读屏豁免和两项延后保留；无新增真实外呼/邮件。
  M6 等待具体 main 合并批准，后续需同 SHA Main/capacity/Release、四镜像、生产
  备份独立恢复、批准切换和至少 24 小时观察；不宣称已部署或补造正式独立 review。

## FABLE M6 合并与制品验收授权（2026-09-10）
