...
| V20-12 | Windows Codex / 已开始、默认关闭                       | 测试与必要修复；Run 任务 `task-v20-12-integration`                                   | 运行下节矩阵；先 bounded 后 repository gates；基础设施缺失记为 unrun，不算通过；不得开启生产能力                        | `check-v12-bounded-negative`、`check-v12-security-integration`、`check-v12-default-closed`、`check-v12-repository-gates`；原始命令、退出码、结果计数、失败现场和 unrun 原因 |
...
下一顺序固定为：V20-12 集成门 → DeepSeek 只读终审 → 回滚演练 → 最终发布。V20-11 已在全部生产能力继续默认关闭的前提下通过；Shared Write、删除、附件、本地 worker、AI Acceptance 生产启用和前端后续 owner 仍不得提前开启。
