...
> 状态：M0 与 V20-01/03/07 已批准，V20-02 隔离迁移证明、V20-04 加法合同门、V20-08 核心实现与 V20-09 AI acceptance 均已完成并由 Codex 验收；V20-10 服务端与前端代码已集成，真实浏览器/移动安全门仍待执行。
...
| V20-10 | Kimi K2.7-code 首版施工 / Windows Codex 集成（进行中） | 前后端路径不重叠；共享合同与最终写入由 Codex                                         | 服务端 1–2 跳/150/400 硬限、bounded search，前端浏览器布局、移动列表/树、只读真实数据适配与安全状态呈现                 | 代码门已通过；仍需真实栈 Playwright、Recall@10、axe/键盘/390px、存储型 XSS 浏览器测试                     |
...
下一顺序固定为：V20-10 真实栈浏览器/移动/安全呈现门 → V20-11 prerequisites 保持默认禁用并完成进入 V20-12 的决策 → V20-12 集成门 → DeepSeek 只读终审 → 回滚演练 → 最终发布。Shared Write、删除、附件、本地 worker、AI Acceptance 生产启用和前端后续 owner 仍不得提前开启。
