...
> 状态：M0 与 V20-01/03/07 已批准，V20-02 隔离迁移证明、V20-04 加法合同门、V20-08 核心实现、V20-09 AI acceptance 与 V20-10 真实栈验收均已完成并由 Codex 验收；当前进入 V20-11 默认关闭准入评审。
...
| V20-06 | 产品 owner + Windows Codex / 首版已集成                | 无并行实现写入                                                                       | 第一版按 Kimi 原型方向施工；用户一次性指定 Kimi K2.7-code 实施，数据/权限/合同与最终集成继续由 Codex 控制                           | Kimi 原始施工 `5d737b7`；Codex 审查修正 `7a93ac9`                                                                                                                                |
...
| V20-11 | Windows Codex / 审查中、硬停止                         | attachment migration/worker security，需单独任务                                     | Resource 绑定仍走安全上传；附件与 Local Worker flag 已在运行时显式默认关闭；本地租约/检查点须独立设计；BitLocker 或等价证明前不启用 | 附件边界/备份单测 7 passed、附件/知识 flag 与默认关闭边界 40 passed、offline 包 55 passed；迁移/恶意文件、加密/ACL、恢复/残留和 worker offline 核心流程证据缺失，不得进入 V20-12 |
...
Kimi 最初只负责第一版整体审批原型及其原型代码。用户随后明确作出一次性覆盖授权，指定 Mac OpenCode Go 的 Kimi K2.7-code 按批准原型完成本次正式前端首版；该实现已由 Windows Codex 接管、审查和集成。授权仍不延续为后续版本迭代的默认 owner，后续模型由用户另行指定。Kimi 不定义共享数据模型、permission、迁移、OpenAPI、AI Provider/路由、保留或一级导航，也不 commit/merge/push。
...
下一顺序固定为：V20-11 prerequisites 保持默认禁用并完成进入 V20-12 的决策 → V20-12 集成门 → DeepSeek 只读终审 → 回滚演练 → 最终发布。Shared Write、删除、附件、本地 worker、AI Acceptance 生产启用和前端后续 owner 仍不得提前开启。
