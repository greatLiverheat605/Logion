...
| V20-11 | Windows Codex / 审查中、硬停止                         | attachment migration/worker security，需单独任务                                     | Resource 绑定仍走安全上传；本地租约/检查点须独立设计；BitLocker 或等价证明前不启用                                      | 附件边界/备份单测 7 passed、offline 包 55 passed；迁移/恶意文件、加密/ACL、恢复/残留和 worker offline 核心流程证据缺失，不得进入 V20-12          |
