...
> 当前阶段：**M0、V20-01/03/07 设计包、V20-02 隔离迁移证明、V20-04 加法合同门、V20-08 核心实现、V20-09 接受闭环、V20-10 真实栈验收、V20-11 默认关闭准入、V20-12 默认关闭集成门、V20-13 DeepSeek 只读终审与 V20-14 隔离回滚演练均已通过 Codex 验收；下一步为 V20-15 最终发布门**。
...
| V20-12～15 集成、终审、回滚、发布 | V20-12～V20-14 已通过，V20-15 待开始 | `codex/v020-integration` 已通过集成门、DeepSeek 终审与隔离回滚演练；敏感能力继续关闭，不等同于生产发布批准                                                                                                   | 进入 V20-15 最终发布门                                            |
...
9. V20-14 隔离回滚演练已完成并接受；下一节点为 V20-15 最终发布门。本次正式首版前端由用户一次性指定 Kimi K2.7-code 完成；未来迭代 owner 仍待用户另行指定。
