...
> 当前阶段：**M0、V20-01/03/07 设计包与 V20-02 隔离迁移证明已通过**。
> 正式实现状态：**V20-02 独立证据复核与验收已收口；V20-04 尚未授权，V20-08 实现门未打开**。
...
PostgreSQL 往返、约束负测、孤儿停止、非空降级停止、备份恢复和合成规模估算。OpenAPI、ORM/核心
服务、生产策略和产品实现仍须分别授权。GLM
纯图内核仍只是无授权、无数据库、无游标和无服务端资源治理的候选，不计 V20-08/V20-10 完成。

不要使用虚构百分比衡量当前进度。按门禁判断：**架构 M0、第一版 UX 方向以及 V20-01/03/07
设计基线与 V20-02 迁移证明已冻结；V20-04 仍待单独授权，正式集成和发布门均未开启。**
...
| V20-01/02 schema 与迁移           | V20-02 隔离证明已完成        | [`V020_MIGRATION_PROOF.md`](./V020_MIGRATION_PROOF.md)；往返/负测/恢复/规模证据已通过；无 commit/push              | 等待 V20-04；ORM 登记和 `alembic check` 收口留给 V20-08           |
| V20-03/04 permission/API/OpenAPI  | V20-03 设计已批准            | [`V020_PERMISSION_API_DESIGN.md`](./V020_PERMISSION_API_DESIGN.md)；用户于 2026-08-05 批准权限与合同设计           | 单独授权 V20-04；当前不得生成/修改 OpenAPI                        |
...
5. 当前停在 V20-04 OpenAPI Gate 审批前；未经单独授权不得生成/修改合同或启动 V20-08。
6. 前端修正继续等待用户指定下一位前端模型；Kimi 不再复用，DeepSeek 仍等 V20-12 终审。
...
- ADR-0029 与 V20-01/03/07 设计基线已获批准，V20-02 仅完成隔离迁移证明；OpenAPI、ORM/服务、
  生产迁移、生产合规证据和敏感能力启用仍分别受独立门禁约束。
- V20-02 未登记 ORM，因此 `alembic check` 如实报告 0036 与元数据的差异；在 V20-08 收口前该候选
  不是可独立合并的 PR。生产锁竞争、真实行数与恢复点也尚未验证。
