...
手动创建目标目前只有首个阶段。收件箱分诊在建立路线/项目成功时原子移出原条目，并支持确认删除；考试支持整组软删除，未关联学习记录或引用的知识点支持确认删除。应用内共享本次 Vault 解锁状态，从成功解锁起 30 分钟到期；手动锁定、退出或刷新后也需重新解锁。已加密保存内容保留，未提交草稿不保证恢复。快速捕获支持保存后继续或关闭，明确区分本地保存与服务器同步。旧模板缓存缺失可通过规划页“补全服务器资料”恢复。
