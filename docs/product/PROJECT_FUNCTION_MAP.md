...
手动创建目标时先建立首个阶段；部署开启阶段编辑（`LOGION_PLANNING_PHASE_REVISION_ENABLED`，默认关闭）后，可对已有目标追加、修改、重排、归档阶段，被任务引用的阶段只能归档。收件箱分诊在建立路线/项目成功时原子移出原条目，并支持确认删除；考试支持整组软删除，未关联学习记录或引用的知识点支持确认删除。应用内共享本次 Vault 解锁状态，从成功解锁起 30 分钟到期；手动锁定、退出或刷新后也需重新解锁。已加密保存内容保留。新建目标的背景说明与验收标准、周期审查总结、回忆题解析和研究声明会在 Vault 解锁时加密保存为本机草稿（ADR-0034），刷新或重新登录后解锁再打开同一表单时询问“恢复未提交内容？”；提交、丢弃、退出登录或 7 天后删除，不同步；其他表单的未提交输入不保证恢复。快速捕获支持保存后继续或关闭，明确区分本地保存与服务器同步。旧模板缓存缺失可通过规划页“补全服务器资料”恢复。
