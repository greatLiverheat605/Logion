...
现在只保留一个由用户指定的主线执行方。其他模型和工具暂停派发；只有用户明确要求或出现特定专业门禁时，协调者才提出模型建议。主线执行方负责实现、测试、问题修复和结构��交接；用户保留产品批准权，协调者保留独立验收和发布门禁权。

## 阶段 0：接手与基线核对（立即）

验收目标：主线执行方能够从文档恢复上下文，不依赖聊天记录。
...
需要用户再次明确批准：真实受邀邮件/设备验收、观察期、最终发布和流量切换。主线执行方可以准备清单和修复，但不能自行执行生产动作。
...
- 主线执行方不得自行 merge/push/release；完成后返回结构化 handoff，等待协调者或用户批准。
