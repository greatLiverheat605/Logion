...
| CSV/BibTeX 公式或语法注入                 | CSV 使用标准 writer，并为 `= + - @ TAB CR` 开头的字符串前置单引号；BibTeX key 字符白名单，花括号从 title/URL 剔除       | 单元测试公式前缀与特殊字符；后续 CSV 导入仍按不可信数据处理 |
...
Phase 5 收口审计进一步验证：没有直接 `space_id` 的 `PlanVersion` 与 `PlanPhase` 必须沿 `PlanVersion -> LearningPlan -> Space` 授权链过滤。集成测试使用其他成员私有 Space 的唯一标记，断言其不进入请求者导出包。
