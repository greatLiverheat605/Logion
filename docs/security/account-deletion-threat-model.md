...
Phase 5 收口审计补充：删除事务先锁定独占 Workspace 的待接受邀请，再复核 active membership；确认可删除后撤销该用户签发的全部 pending 邀请。邀请接受与删除请求锁定同一邀请行，避免宽限期内新成员加入后被后续清理波及。集成测试验证旧邀请 Token 在删除请求后不可使用。
