...
- 笔记选段创建的知识点和回忆题可回到原文：复习页显示“来源”及有效、已修改、已删除、不可用四种状态，记录页列出“由此创建”并定位原选段；链接只保存 ID、偏移和摘要哈希，删除笔记不受阻止。由 `LOGION_SOURCE_LINKS_ENABLED` 控制，默认关闭。
- 新建目标、周期审查、回忆题解析和研究声明的长文本在 Vault 解锁时加密保存为本机草稿；刷新或重新登录后解锁可选择恢复或丢弃，提交、丢弃、退出登录或 7 天后删除，不同步。
- 离线打开页面时的兜底页说明原因、本机资料仍在以及联网后如何继续；离线也能正常显示样式，“重新打开此页”不依赖脚本。
