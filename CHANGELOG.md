...
- 已有目标可编辑路线：追加、修改、键盘重排、归档或恢复阶段，被任务引用的阶段只能归档；由 `LOGION_PLANNING_PHASE_REVISION_ENABLED` 控制，默认关闭。
- 同一浏览器标签页中恢复今日、规划、记录、复习、备考、模板、自学、研究和协作页的工作区与空间（搜索页恢复工作区），复习页另恢复当前标签和选中知识点；只保存对象 ID，退出登录时清除。
