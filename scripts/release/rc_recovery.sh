...
mkdir -p "${attachments}"
printf '%s' 'logion-rc-attachment-marker-v1' >"${attachments}/restore-marker.txt"
...
source_attachment_sha="$(sha256sum "${attachments}/restore-marker.txt" | cut -d ' ' -f 1)"
restored_attachment_sha="$(docker exec "${backup_container}" sha256sum /tmp/logion-rc-restore-attachments/restore-marker.txt | cut -d ' ' -f 1)"
...
jq -n \
  ...
  '{
    ...
    restored_counts: {users: $users, workspaces: $workspaces, memberships: $memberships, audit_events: $audit_events, attachments: 1},
